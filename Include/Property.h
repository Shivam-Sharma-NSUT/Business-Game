...
class PropHolder{//optimized
    ...
    drawprop(int i,int player){
        ...
        setcolor(BLACK);
        switch(player){
        case 1:     if(prop[list1[i]].prop_rate==0)return 0;
                    setbkcolor(prop[list1[i]].prop_color);
            ...
            break;
            case 2: if(prop[list2[i]].prop_rate==0)return 0;
                    setbkcolor(prop[list2[i]].prop_color);
