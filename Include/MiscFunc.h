...
void Save(){
    data.p1cash=player1.cash;
    data.p2cash=player2.cash;
    strcpy(data.p1name,player1.name);
    strcpy(data.p2name,player2.name);
    data.p1pos=player1.token.position;
    data.p2pos=player2.token.position;
    for(int i=0;i<15;i++){
        data.p1list[i]=list1[i];
        data.p2list[i]=list2[i];
        data.p1propstatus[i]=prop[list1[i]].prop_status;
        data.p1propmultiplier[i]=prop[list1[i]].multiplier;
        data.p2propstatus[i]=prop[list2[i]].prop_status;
        data.p2propmultiplier[i]=prop[list2[i]].multiplier;
    }
    sfile=fopen("Save.BGR.BAK","wb");
    fwrite(&data,sizeof(Data),1,sfile);
    fclose(sfile);
    system("del Save.BGR");
    system("ren Save.BGR.BAK Save.BGR");
}
...
void Resume(){
    ...
    init_Grid();
    DrawHold();
    sfile=fopen("Save.BGR","rb");
    fread(&data,sizeof(Data),1,sfile);
    player1.dice.color=data.d1;
    player2.dice.color=data.d2;
    player1.token.color=data.t1;
    player2.token.color=data.t2;
    player1.token.position=data.p1pos;
    player2.token.position=data.p2pos;
    player1.cash=data.p1cash;
    player2.cash=data.p2cash;
    player1.token.position=data.p1pos;
    player2.token.position=data.p2pos;
    strcpy(player1.name,data.p1name);
    strcpy(player2.name,data.p2name);
    for(int i=0;i<15;i++){
        list1[i]=data.p1list[i];
        list2[i]=data.p2list[i];
        prop[list1[i]].prop_status=data.p1propstatus[i];
        prop[list1[i]].multiplier=data.p1propmultiplier[i];
        prop[list2[i]].prop_status=data.p2propstatus[i];
        prop[list2[i]].multiplier=data.p2propmultiplier[i];
        holdpos1[p1top].drawprop(p1top,1);
        if(list1[i])
            p1top++;
        prop[list1[i]].drawFlags(player1.token.color);
        holdpos2[p2top].drawprop(p2top,2);
        if(list2[i])
            p2top++;
        prop[list2[i]].drawFlags(player2.token.color);
    }
    fclose(sfile);
    player1.token.draw((prop[player1.token.position].x1+prop[player1.token.position].x2)/2,(prop[player1.token.position].y1+prop[player1.token.position].y2)/2);
    player2.token.draw((prop[player2.token.position].x1+prop[player2.token.position].x2)/2,(prop[player2.token.position].y1+prop[player2.token.position].y2)/2);
    ActualDice();
}
